-- Seed para la tabla artworks (y artists) en entorno local.
-- Ejecutar después de las migraciones (005_create_artworks.sql).
-- Uso: Supabase Studio (SQL Editor) o: psql $DATABASE_URL -f supabase/seed_artworks.sql
-- Con Supabase CLI: supabase db reset (usa supabase/seed.sql).

-- 1) Artistas. Si la tabla tiene slug (migración 002): ON CONFLICT. Si no tiene slug (solo 005): insertar solo (name, bio, photo).
DO $$
BEGIN
